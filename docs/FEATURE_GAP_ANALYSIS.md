...
> 產出日期：2026-06-22
> ⚠️ **歷史快照**：本報告完成後專案大幅推進（v0.50→v0.65：Compress 分頁、捲動截圖、
> 高亮/步驟/Callout 標註、歷史搜尋、toast、Linux 移植、跨平台自動更新等），
> 多數 P0/P1 項目已出貨。閱讀時請以 `CHANGELOG.md` 為準。
...
1. ~~**一個高風險的正確性 Bug**：暫停/續錄後的多分段錄影只會保留第一段，**其餘內容被靜默丟棄**。~~
   ✅ 已修復（`LibavMuxer.ConcatVideoSegments`，見 §1.1）— 摘要保留供歷史對照。
