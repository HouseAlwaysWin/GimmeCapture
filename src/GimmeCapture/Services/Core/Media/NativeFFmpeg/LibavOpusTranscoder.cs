...
/// <summary>
/// WAV -> Opus-in-OGG for muxing into WebM (native libav*, no ffmpeg.exe).
/// </summary>
internal static class LibavOpusTranscoder
{
    public static unsafe void EncodeWavToOpusOgg(string wavPath, string oggPath, VideoQuality quality)
    {
        ...
        try
        {
            AVCodec* enc = ffmpeg.avcodec_find_encoder_by_name("libopus");
            if (enc == null)
            {
                enc = ffmpeg.avcodec_find_encoder(AVCodecID.AV_CODEC_ID_OPUS);
            }

            if (enc == null)
            {
                throw new InvalidOperationException("libopus encoder unavailable.");
            }

            encCtx = ffmpeg.avcodec_alloc_context3(enc);
            if (encCtx == null)
            {
                throw new OutOfMemoryException("opus_alloc_enc");
            }

            encCtx->sample_rate = provider.WaveFormat.SampleRate;
            encCtx->time_base = new AVRational { num = 1, den = encCtx->sample_rate };
            ThrowIfErr(ffmpeg.av_channel_layout_copy(&encCtx->ch_layout, &stereoLayout), "opus_enc_layout");
            encCtx->sample_fmt = ChooseSampleFormat(enc);
            encCtx->bit_rate = quality switch
            {
                VideoQuality.High => 192_000,
                VideoQuality.Low => 64_000,
                _ => 128_000,
            };

            ThrowIfErr(ffmpeg.avcodec_open2(encCtx, enc, null), "opus_open_enc");

            ThrowIfErr(ffmpeg.avformat_alloc_output_context2(&outFmt, null, "ogg", oggPath), "opus_alloc_out");
            if (outFmt == null)
            {
                throw new InvalidOperationException("Failed to create OGG output context.");
            }

            outStream = ffmpeg.avformat_new_stream(outFmt, null);
            if (outStream == null)
            {
                throw new OutOfMemoryException("opus_new_stream");
            }

            ThrowIfErr(ffmpeg.avcodec_parameters_from_context(outStream->codecpar, encCtx), "opus_par_from_ctx");
            outStream->codecpar->codec_tag = 0;
            outStream->time_base = encCtx->time_base;

            if ((outFmt->oformat->flags & ffmpeg.AVFMT_NOFILE) == 0)
            {
                ThrowIfErr(ffmpeg.avio_open(&outFmt->pb, oggPath, ffmpeg.AVIO_FLAG_WRITE), "opus_avio_open");
            }

            ThrowIfErr(ffmpeg.avformat_write_header(outFmt, null), "opus_write_header");

            outPkt = ffmpeg.av_packet_alloc();
            if (outPkt == null)
            {
                throw new OutOfMemoryException("opus_alloc_pkt");
            }

            int channels = provider.WaveFormat.Channels;
            int frameSamples = encCtx->frame_size > 0 ? encCtx->frame_size : 960;
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    ptsSamples += frameSamples;
                    ThrowIfErr(ffmpeg.avcodec_send_frame(encCtx, frame), "opus_send_frame");
                    OpusWriteEncodedPackets(encCtx, outFmt, outStream, outPkt);
                }
                finally
                {
                    if (frame != null)
            ...
            ThrowIfErr(ffmpeg.avcodec_send_frame(encCtx, null), "opus_flush_enc");
            OpusWriteEncodedPackets(encCtx, outFmt, outStream, outPkt);
    ...
    private static unsafe AVFrame* CreateAudioFrame(AVCodecContext* encCtx, AVChannelLayout* layout, int sampleCount)
    {
        ...
        if (frame == null)
        {
            throw new OutOfMemoryException("opus_alloc_frame");
        }
        ...
        try
        {
            ...
            ThrowIfErr(ffmpeg.av_channel_layout_copy(&frame->ch_layout, layout), "opus_frame_layout");
            ThrowIfErr(ffmpeg.av_frame_get_buffer(frame, 0), "opus_frame_buffer");
            ThrowIfErr(ffmpeg.av_frame_make_writable(frame), "opus_frame_writable");
            return frame;
        }
        catch
        {
        ...
        int channels)
    {
        ...
        fixed (float* src = interleavedSamples)
        {
            switch (sampleFormat)
            {
                ...
                default:
                    throw new NotSupportedException($"Unsupported Opus encoder sample format: {sampleFormat}");
            }
        }
    }
    ...
    private static unsafe void OpusWriteEncodedPackets(
        ...
        AVPacket* outPkt)
    {
        while (true)
        {
            ...
            ThrowIfErr(gp, "opus_receive_packet");
            ffmpeg.av_packet_rescale_ts(outPkt, encCtx->time_base, outStream->time_base);
            outPkt->stream_index = outStream->index;
            ThrowIfErr(ffmpeg.av_interleaved_write_frame(outFmt, outPkt), "opus_write_frame");
